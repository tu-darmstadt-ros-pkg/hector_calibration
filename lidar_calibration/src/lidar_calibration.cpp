...
bool LidarCalibration::saveToDisk(std::string path, const Calibration& calibration) const {
  ...
  outfile <<
      "<!-- =================================================================================== -->" << std::endl <<
      "<!-- |    This document was autogenerated.                                             | -->" << std::endl <<
      "<!-- |    Insert this transformation between frames " << actuator_frame_ << " and " << laser_frame_ <<  ". | -->" << std::endl <<
  ...
  outfile << "  <origin rpy=\"" << calibration.roll << " " << calibration.pitch << " " << calibration.yaw << "\" ";
  outfile << "xyz=\"" << calibration.x << " " << calibration.y << " " << calibration.z << "\"/>" << std::endl;
  outfile << "</robot>";
  outfile.close();
  return true;
}
